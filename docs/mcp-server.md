...
**Resources and instructions.** Every session carries `instructions`: a short preamble about this server followed by the editor skill's `SKILL.md`, read from the staged skills folder, so an agent that connects knows how to work with the app without a separate skills install. The staged docs are served as resources (`dapi://docs/reference/…`, `dapi://docs/examples/…`) alongside the skill's references (`dapi://skills/editor/…`), and three live ones: `dapi://context` (what `context` returns), `dapi://logs`, and `dapi://project/AGENTS.md` for the open project. Implemented in `apps/desktop/src/dapi/knowledge.ts`.
...
*Superseded on `mcp-support`: with the server carrying the skill as `instructions` and the docs as resources, the two external copies are gone. The app no longer symlinks skills into agent directories (`skills-install.ts` is deleted, with its onboarding row), and it no longer copies `reference/` and `examples/` into every project's `.diffusion/docs` (the scaffold removes a copy left by an earlier version). A project's `AGENTS.md` now points at the server: the URL, the resource URIs to read first, and the CLI as the no-MCP fallback. Only the staged copies inside the app bundle remain, and they are what the server serves, so an agent can only ever read the installed version's docs. The skill text itself still comes from the skills repo at staging time and is what step 5 updates.*
