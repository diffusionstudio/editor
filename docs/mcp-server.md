...
**Resources and instructions.** Every session carries `instructions`: `knowledge/INSTRUCTIONS.md`, an orientation and an index of the skills to read, so an agent that connects knows how to work with the app without a separate skills install. The rest of the knowledge base is served as resources under `dapi://<path>` (`dapi://reference/…`, `dapi://examples/…`, `dapi://guides/…`, `dapi://brand/…`, `dapi://skills/…`), and two live ones: `dapi://context` (what `context` returns) and `dapi://logs`. Implemented in `apps/desktop/src/dapi/knowledge.ts`.

**HTTP.** Beside the socket, main serves the same sessions over Streamable HTTP at `http://127.0.0.1:3274/mcp` (`apps/desktop/src/dapi/http.ts`; the constants live in `@diffusionstudio/dapi/socket`). One MCP server per session, keyed by the transport's session id; the SDK's DNS-rebinding protection is on with the loopback hosts allowed. This is the transport agents register: a URL that is the same on every machine.
...
*Superseded on `mcp-support`: with the server carrying the skill as `instructions` and the docs as resources, the two external copies are gone. The app no longer symlinks skills into agent directories (`skills-install.ts` is deleted, with its onboarding row), and it no longer copies `reference/` and `examples/` into every project's `.diffusion/docs` (a copy left by an earlier version is left alone). Projects get no `AGENTS.md` any more; their README says everything goes through the server and gives the URL, which is enough for an agent that lacks it to add it. Only the staged copies inside the app bundle remain, and they are what the server serves, so an agent can only ever read the installed version's docs. The knowledge base now lives in this repo under `knowledge/` (instructions, skills, guides, reference, examples, brand) and is staged whole by `scripts/stage-knowledge.mjs`; the separate skills repo is no longer a source.*
...
4. **CLI.** Add `dapi mcp` and `dapi call`, and generate the wrappers from the catalog. Update `cli-install.ts` and the settings UI to offer MCP registration. *Landed on `mcp-support`, with two changes. `dapi call` was dropped. The CLI ships inside the app, so it always matches the app's catalog and every tool has a wrapper; a generic client would only duplicate what an MCP inspector on `dapi mcp` shows. `dapi mcp` is a byte pipe in `apps/cli/src/mcp-proxy.ts`; every wrapper's description and option help comes from the catalog, the input is validated locally with the same schemas, and the tool's structured content is printed unchanged. And registration writes the HTTP URL, not the proxy: the app registers `http://127.0.0.1:3274/mcp` with every agent present on the machine that speaks Streamable HTTP (Claude Code, Codex, Cursor, Gemini CLI, Windsurf) and `<bundle>/Contents/Resources/cli/bin/dapi mcp` with the one that does not (Claude Desktop), each under the keys its config format uses (`apps/desktop/src/mcp-install.ts`, merge rules in `mcp-config.ts`), from the onboarding screen and a menu item. The PATH symlink install stays as its own onboarding row and menu item.*
5. **Docs.** Update the skill and regenerate `reference/`. Update the installation reference the skill points to. *Landed on `mcp-support`. The knowledge base is written for both surfaces: tools are named as MCP lists them, and the naming rule that turns a tool into a `dapi` command (`media_grab` is `dapi media grab`, `perSheet` is `--per-sheet`) is stated once in `knowledge/reference/tools/README.md` and in the server instructions. Each tool page opens with the catalog description verbatim, the MCP name and CLI synopsis, and a table of the input fields with the flag that carries each; the prose after it is surface-neutral. The option tables are kept in step with the catalog by hand.*
6. **Cleanup.** Remove `CLI_WIRE`, the handshake types, and the `./protocol` and `./channels` exports from the CLI package. *Landed on `mcp-support`.*
...
- **HTTP transport.** *Resolved: the app serves Streamable HTTP on a fixed loopback port (`http://127.0.0.1:3274/mcp`, see part 2) beside the socket, and that URL is what agents register. No token: the server binds to loopback and rejects any `Host` other than the loopback address, which is what keeps browser pages out; other processes running as the user can already reach the socket. `dapi mcp` stays for clients that only spawn stdio servers (Claude Desktop). The cost accepted: an agent whose app is closed gets connection refused and must run `dapi open` first, and a taken port means no HTTP endpoint until it is freed (the socket keeps working; the app logs it).*
