...
export function BackgroundSettings() {
  ...
  // Layer-less inspector: no owning node, so it keeps normal outside-dismiss
  // behavior (no timeline-session policy).
  const backgroundSession = (): InspectorSession => ({
    owner: OWNER,
    id: SLOT,
    ownerNodeEid: null,
      ...
      <PanelSection title="Background" ref={anchorRef}>
        <ControlRow label="Color">
          <ColorOpacityRow
            ...
            onClick={() => inspectors.toggle(backgroundSession())}
          />
        </ControlRow>
      </PanelSection>
      </div>
